...
//
//				Map<Integer, List<MyFile>> myValues = AppConfig.chordState.getValueMap();
//				Map<Integer, List<MyFile>> hisValues = new HashMap<>();
//
//				int myId = AppConfig.myServentInfo.getChordId();
//				int hisPredId = hisPred.getChordId();
//				int newNodeId = newNodeInfo.getChordId();
//
//				for (Entry<Integer, List<MyFile>> valueEntry : myValues.entrySet()) {
		...
		try {
			...
			if (clientMessage.getMessageType() == MessageType.NEW_NODE) {
				...
				if (isMyPred) { //if yes, prepare and send welcome message
					...
					Map<Integer, List<MyFile>> myValuesData = AppConfig.chordState.getValueMap();
					...
					AppConfig.chordState.setValueMap(myValuesData);

					WelcomeMessage wm = new WelcomeMessage(AppConfig.myServentInfo.getListenerPort(), newNodeInfo.getListenerPort(), hisValuesData);
					MessageUtil.sendMessage(wm);

				} else { //if he is not my predecessor, let someone else take care of it

					AppConfig.timestampedStandardPrint("New NodeInfo: " + newNodeInfo.toString());
//					AppConfig.timestampedStandardPrint("Succesors" + AppConfig.chordState.getSuccessorTable().toString());
					AppConfig.timestampedStandardPrint("Succesors");
					for (ServentInfo serventInfo : AppConfig.chordState.getSuccessorTable()) {
						AppConfig.timestampedStandardPrint("Succesor " + serventInfo.toString());
					}
					ServentInfo nextNode = AppConfig.chordState.getNextNodeForKey(newNodeInfo.getChordId());

					NewNodeMessage nnm = new NewNodeMessage(newNodeInfo.getListenerPort(), nextNode.getListenerPort(), rbs);
					MessageUtil.sendMessage(nnm);
				}
